...
| Version and ABI manifests | `release.toml`, `bindings/versions.toml`, `bindings/api_manifest.toml` | Coordinated package versions and publication order, binding version coordination, and the C ABI surface that must remain compatible. Validate with `tools/release/validate_release_manifest.py`. |
...
- `release.toml` records the coordinated release, every workspace package,
  independent package-family versions, binding/native artifact versions, and
  dependency-safe publication order. It must agree with Cargo metadata.
- `bindings/versions.toml` remains the input for binding synchronization and
  must agree with the binding values in `release.toml`.
